...
function comment_pagination($pager, $uri, $sf_params, $prev_text = 'Previous', $next_text = 'Next', $sort = 'desc')
{
  $navigation = '';
    ...
    {
      if ($pager->getPage() != 1) {
        $navigation .= '<div class="nav-previous"><span class="meta-nav">&larr;</span><a href="'.formatUrlFromParameters($uri.$pager->getPreviousPage(), $prms).'" class="comment-link">'.__($prev_text).'</a></div>';
      }
      if ($pager->getPage() != $pager->getLastPage()) {
        $navigation .= '<div class="nav-next"><a href="'.formatUrlFromParameters($uri.$pager->getNextPage(), $prms).'" class="comment-link">'.__($next_text).'</a><span class="meta-nav">&rarr;</span></div>';
      }
    }
    else
    {
      if ($pager->getPage() != 1) {
      $navigation .= '<div class="nav-next"><a href="'.formatUrlFromParameters($uri.$pager->getPreviousPage(), $prms).'" class="comment-link">'.__($next_text).'</a><span class="meta-nav">&rarr;</span></div>';
      }
      if ($pager->getPage() != $pager->getLastPage()) {
        $navigation .= '<div class="nav-previous"><span class="meta-nav">&larr;</span><a href="'.formatUrlFromParameters($uri.$pager->getNextPage(), $prms).'" class="comment-link">'.__($prev_text).'</a></div>';
      }
    }
    $navigation .= '</div>';
  }
